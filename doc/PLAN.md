...
5. **Email layer** — harden `send_email()` helper (singleton client, async offload, tenacity retry, Dynamic Templates, structured logging), wire SendGrid domain auth + staging secrets, then the verification signed-token flow and verification frontend pages. Each merge to `main` auto-deploys to staging.
