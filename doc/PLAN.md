...
5.5. **Auth session lifecycle hardening** — surfaced during 0015 staging verification and 0016 password-reset rollout. Forgot/reset pages, Cloud Run GFE rate-limiter IP fix, cross-device ghost-session handling, proxy-symmetric redirect pair, unified pre-decision blurry `LoadingScreen`, OAuth 2.0 `expires_in` + proactive-refresh scheduler, logout round-trip cleanup, distinct 401 error codes. ✅ done (tickets 0016, 0016.1–0016.8, all on main as of 2026-04-23). Chassis outcomes captured in `doc/systems/auth.md` (§Proactive refresh, §API error codes). Phase 2 staging sanity check still to run post-deploy.
6. **Stripe layer** — create Customer on signup, PAYG Payment Intent endpoint, credit ledger, webhook handler, optional subscription endpoints. Auto-deploys to staging on merge. **Chassis-first reframe (2026-04-23)** moved all project-specific pricing out of `doc/systems/payments.md` (payments is now the third chassis subsystem after auth + email — USD balance with micro-dollar precision, append-only ledger, Stripe-driven tier metadata, project-agnostic primitives). Sub-sequence reshape (2026-04-23): billing UI needed a nav home, so the app-shell refactor (0022) landed before the billing endpoints (0023), preserving clean substrate layering. **Done:** 0021 (foundation — ledger schema, `app.billing` primitives, webhook skeleton, conditional register/verify hooks, `BILLING_ENABLED=false` default; on dev + main + staging 2026-04-23) ✅; 0022 (app-shell refactor — left-rail sidebar + profile popover; 2026-04-23) ✅; 0023 + 0023.1 + 0023.2 (PAYG topup chassis — `/billing/topup` + `/billing/balance` + `payment_intent.succeeded` handler + Stripe Elements TopupForm; test-isolation discipline; webhook dedup race fix; live test-mode topup verified on staging 2026-04-25) ✅. **Next tickets:** **0024** subscribe + subscription lifecycle handlers; **0025+** Customer Portal + pricing page + /app integration. All carddroper-specific pricing decisions (tier count, preset topups, bonus amounts, action debit costs) deferred until the chassis is complete.
