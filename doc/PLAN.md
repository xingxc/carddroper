...
6. **Stripe layer** — create Customer on signup, PAYG Payment Intent endpoint, credit ledger, webhook handler, optional subscription endpoints. Auto-deploys to staging on merge. **In progress:** chassis-first reframe on 2026-04-23 moved all project-specific pricing out of `doc/systems/payments.md` (payments is now the third chassis subsystem after auth + email — USD balance with micro-dollar precision, append-only ledger, Stripe-driven tier metadata, project-agnostic primitives). Ticket 0021 (foundation — ledger schema, `app.billing` primitives, webhook skeleton with signature + replay idempotency, conditional register/verify hooks, `BILLING_ENABLED=false` default) ✅ done 2026-04-23 on dev + main + staging. Sub-sequence reshape on 2026-04-23: billing UI needs a nav home, so app-shell refactor (Canva/Stripe-style left rail + profile popover) landed as its own ticket (0022) before the billing endpoints (now 0023), preserving clean substrate layering. Next tickets: **0022** app-shell refactor (left-rail sidebar + profile popover); **0023** PAYG topup + `GET /billing/balance` + `payment_intent.succeeded` handler; **0024** subscribe + subscription lifecycle handlers; **0025+** Customer Portal + pricing page + /app integration. All carddroper-specific pricing decisions (tier count, preset topups, bonus amounts, action debit costs) deferred until the chassis is complete.
