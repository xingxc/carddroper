...
5. **Stripe layer** — create Customer on signup, PAYG Payment Intent endpoint, credit ledger, webhook handler, optional subscription endpoints. Each merge to `main` auto-deploys to staging.
6. **Email verification** — SendGrid template, signed-token flow, frontend pages. Auto-deploys to staging on merge.
