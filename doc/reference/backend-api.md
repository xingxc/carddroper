...
| POST | `/billing/subscribe` | access + verified; rate-limited (`SUBSCRIBE_RATE_LIMIT`, default 5/min per IP) | Create a Stripe Subscription. Request: `{price_lookup_key: str, payment_method_id: str}`. Response: `{subscription_id: str, status: str, requires_action: bool, client_secret: str \| null}`. Resolves Price by lookup_key; validates `metadata.tier_name` (always required) and `metadata.grant_micros` (required only when `BILLING_SUBSCRIPTION_GRANTS_TO_LEDGER=true`; optional when false); attaches PM; creates Subscription; upserts `subscriptions` row. Returns 404 if Price not found; 422 if required metadata missing; 409 if already active/trialing/past_due. `requires_action=true` when Stripe returns `status='incomplete'` (3DS challenge). Balance grant deferred to `customer.subscription.created` webhook (only fires when `BILLING_SUBSCRIPTION_GRANTS_TO_LEDGER=true`). |
...
| GET  | `/billing/tiers` | access (not verified-gated) | Return enriched tier envelopes for the requested Stripe Price lookup_keys. Query param: `lookup_keys` (CSV string; empty or absent → `[]`). Calls `stripe.Price.list(lookup_keys=..., active=True, expand=["data.product"])` in a single round-trip. Response: `list[TierEnvelope]` where each item is `{lookup_key: str, tier_name: str, description: str \| null, price_display: str, amount_cents: int, currency: str, interval: str, interval_count: int, grant_micros: int}`. Prices missing `metadata.tier_name` or `metadata.grant_micros` are silently skipped (structured warning logged). Non-USD currency logs warning; tier still returned with `$` prefix (USD-only chassis v1 policy). Response order matches input `lookup_keys` order. No rate limit. |
